...
function hydrateHeroCarousel() {
  ...
  const cardHtml = (p, extraClass = '') => `
    <div class="hero-card${extraClass ? ' ' + extraClass : ''}" onclick="navigate('property','${p.id}')">
      <div class="hc-img">
        <img src="${p.image || fallbackImg}" alt="${p.addr || ''}" loading="lazy" onerror="this.src='${fallbackImg}'" />
      </div>
      <div class="hc-tag">${tagLabel(p)}</div>
      <div class="hc-body">
        <div class="hc-price">${p.price}</div>
        <div class="hc-addr">${lucideIcon('mapPin')} ${p.addr || '—'}</div>
        <div class="hc-meta">
          ${p.rooms ? `<span>${lucideIcon('bed')} ${p.rooms} комн</span>` : ''}
          ${p.area ? `<span>${lucideIcon('ruler')} ${p.area} м²</span>` : ''}
          ${p.year ? `<span>${lucideIcon('construction')} ${p.year}</span>` : ''}
          ${p.floor ? `<span>${lucideIcon('building')} ${p.floor} эт</span>` : ''}
        </div>
      </div>
    </div>`;

  // First card gets the main (large) style
  track.innerHTML = listings.map((p, i) => cardHtml(p, i === 0 ? 'hc-main' : '')).join('');
}
...
const REQUEST_TYPES = {
  ...
  buyout: {
    ...
    title: 'Срочный выкуп',
    subtitle: 'Оставьте заявку — мы быстро выкупим вашу недвижимость.',
  },
};
...
function saveFavoriteIds(ids) {
  saveFavorites([...new Set(ids.map(String))]);
}
